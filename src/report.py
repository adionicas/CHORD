...
def _excl_feat_block(excl_feat_list, multimodal):
    ...
    if multimodal:
        def _measure(c):
            ...
            return "(unknown)"
        d = d.copy()
        d["Measure group"] = d[name_col].astype(str).map(_measure)
        rows = []
        for meas, g in d.groupby("Measure group"):
            rows.append({
                "Measure group":            meas,
                "n excluded":               len(g),
...
def _feature_group_block(feature_cols, modality_groups):
    """S1.1 table: all harmonized features grouped by measure group (one row per
    measure, the ROI list in a single cell), mirroring Table S1a. Falls back to a
    pill list when the features do not split into two or more groups."""
    if not feature_cols:
        return ""

    def _measure(c):
        if modality_groups:
            for g, cols in modality_groups.items():
                if c in cols:
                    return g
        if "." in c:
            return c.rsplit(".", 1)[1]
    ...
    for c in feature_cols:
        grouped.setdefault(_measure(c), []).append(c)
    if len(grouped) <= 1:
        return _format_feature_list(feature_cols)
    rows = [{
        "Measure group":   meas,
        "n features":      len(cols),
        "ROIs (features)": ", ".join(cols),
    } for meas, cols in grouped.items()]
    return _df_table(pd.DataFrame(rows))
...
<hr class="divider">
<h1>Multi-batch ComBat Harmonization Assessment</h1>
<p class="authors">
...
<div style="background:#f0f4f8;border-left:4px solid #2A6EBB;border-radius:4px;
  ...
  <p style="font-size:11pt;font-family:'Times New Roman',serif;line-height:1.75;
    ...
    (1) batch mean <em>z</em>-score deviation from the grand mean before and after harmonization,
    assessing residual batch-related variability;
    (2) analysis of covariance (ANCOVA, Type II sums of squares; covariates: {{ age_col }}, {{ sex_col }})
    with batch as the grouping factor, quantifying batch effect size as Cohen's <em>f</em> [4];
...
<p class="table-title">Table S1. Sample characteristics by batch.</p>
{{ demo_table }}
<p class="table-note">
...
<p>
  ...
  <em>neuroCombat</em> Python package (v0.2.12). ComBat models batch-related
  ...
  to refine the estimation of batch-specific location and scale parameters,
  which can stabilize estimates particularly in batches with small sample sizes
  or low within-batch variability [<a href="#ref1">1</a>]. When EB is disabled
...
<p>
  ...
  respective effects on batch-effect reduction and within-batch consistency
  preservation.
  {% else %}
...
<p>
  ...
  information across features to estimate each batch's location and scale
...
<p>
  ...
  that together assess (a) the magnitude of batch-related variability before and
  after harmonization, (b) the extent to which original within-batch variability
...
<h3>S3.1 Batch Effect Quantification</h3>
<p>
  ...
  across all participants) and the mean <em>z</em>-score was computed per batch.
  Successful harmonization should reduce batch mean <em>z</em>-scores toward zero.
  {% if include_cohens_f %}
  Additionally, for each imaging feature a one-way analysis of covariance (ANCOVA) model
  was fitted with batch as the grouping factor and age and sex as covariates,
  using Type II sums of squares. The proportion of variance attributable to batch
...
<h3>S3.2 Within-Batch Consistency</h3>
<p>
  Within-batch consistency was evaluated as the degree to which the rank ordering
  ...
  within-batch biological variability [<a href="#ref3">3</a>].
</p>
...
<h3>S4.1 Batch Effects Before and After Harmonization</h3>
<p>
  ...
  showing large batch effects (Cohen's <em>f</em> &#8805; 0.40),
  ...
  {% endif %}
  {% endif %}
  Batch mean <em>z</em>-score deviations from the grand mean are shown in Figure S1;
  deviations should approach zero after effective harmonization.
</p>

<div class="fig-wrap">{{ fig_site_dev }}</div>
<p class="fig-caption">
  <b>Figure S1.</b> Batch mean deviation from the grand mean before and after harmonization.
  For each imaging feature, values were standardized to a <em>z</em>-score
  (grand mean = 0, grand SD = 1) and the batch mean was computed.
  Each point represents one (batch, feature) pair. After effective harmonization,
  batch means should cluster near zero. Diamond marker = mean across all features
  for each batch.
</p>
...
<p class="fig-caption">
  <b>Figure S2.</b> Batch effect size (Cohen's <em>f</em>) before versus after harmonization.
  Each point represents one imaging feature. Points below the diagonal indicate
  a reduction in batch effect following harmonization. Reference lines on both axes
  ...
  Statistical significance of the batch effect is the uncorrected ANCOVA
  ...
  large batch effects may produce values substantially above 1.0.
</p>
{% endif %}

<h3>S4.2 Within-Batch Consistency</h3>
<p>
  ...
  {% endif %}
  The per-batch breakdown is shown below.
</p>

{% set ns = namespace(fig = (2 if (include_cohens_f and fig_cohens_f) else 1)) %}
{% if fig_icc_site %}
{% set ns.fig = ns.fig + 1 %}
<div class="fig-wrap">{{ fig_icc_site }}</div>
<p class="fig-caption">
  <b>Figure S{{ ns.fig }}.</b> Within-batch consistency by batch: ICC3 (two-way mixed effects, consistency) between raw and harmonized values,
  computed separately for each batch. Batch labels include participant count (n).
  Each box shows the distribution of ICC3 values across all features within that batch.
  Batches with smaller sample sizes may show lower consistency, particularly when
...
<p class="fig-caption">
  <b>Figure S{{ ns.fig }}.</b> ICC(C,1) 95% confidence intervals under EB=TRUE versus EB=FALSE,
  for the same features summarised in the figure above. Each feature contributes two vertical
  segments, its 95% confidence interval for the raw-versus-harmonized ICC(C,1) under EB=TRUE and
  under EB=FALSE, drawn at a shared position on the x-axis so the two configurations are compared
  for the identical feature. Features are ordered by their EB=TRUE ICC. These are overall ICC(C,1)
  values computed across all participants, not within-batch values. Where a feature's two intervals
  overlap in height, their confidence intervals overlap, indicating the two configurations do not
  differ meaningfully for that feature.
</p>
{% endif %}
...
<p>
  ...
  shown in Figure S{{ ns.fig }}.
</p>

<div class="fig-wrap">{{ fig_age }}</div>
<p class="fig-caption">
  <b>Figure S{{ ns.fig }}.</b> Age associations before versus after harmonization.
...
<p class="table-note">
  ...
  Cohen's <em>f</em> = batch effect size from ANCOVA (Age + Sex as covariates, Type II SS).
  Cohen's <em>f</em> before = pre-harmonization batch effect; Cohen's <em>f</em> after = post-harmonization batch effect.
</p>
...
<p class="table-note">
  ...
  Cohen's <em>f</em> after = post-harmonization batch effect size (lower is better).
...
<div class="footer">
  ...
  {{ n_features }} features &nbsp;|&nbsp; {{ n_sites }} batches
</div>
...
def build_methods_paragraph(
    ...
    metrics = [
        "(1) batch mean z-score deviation from the grand mean before and after harmonization, "
        "assessing residual batch-related variability"
    ]
    metric_idx = 2
    ...
    if include_icc_by_site:
        metrics.append(
            ...
            f"computed within each batch between pre- and post-harmonization values for each feature, "
            ...
            f"preserved within-batch biological variability"
        )
        metric_idx += 1
    ...
    if include_cohens_f:
        metrics.append(
            ...
            f"covariates: {age_col}, {sex_col}) with batch as the grouping factor, "
            f"quantifying batch effect size as Cohen's f (Cohen, 1988)"
        )
        metric_idx += 1
...
def generate_report(
    ...
    html = tmpl.render(
        ...
        feature_group_table = _feature_group_block(feature_cols, modality_groups),
